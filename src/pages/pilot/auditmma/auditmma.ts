...
/**
 * Generated class for the AuditmmaPage page.
 *
...
@IonicPage()
@Component({
  selector: "page-auditmma",
  templateUrl: "auditmma.html"
})
export class AuditmmaPage {
  actionPlan: any;
  text: any;
  ...
  rating: any;
  ShowNext: boolean;
  // Variables which were not declared but used
  subActivityData = "";
  supplierData = "";
  projectData = "";
  ...
  projectChange(event) {
    ...
    if (event != "") {
      console.log("event", event);
      this.projectList.map((project: any) => {
        if (project.projectId == event) {
  ...
  continue(audit) {
    ...
    if (this.targetList.length > 0) {
      this.navCtrl.push(FirewallPage, { auditdata: "audit", activity: "MMA" });
    } else {
    ...
    this.commonServiceProvider.updateRating(inputdata).subscribe(
      res => {
        if (res.success) {
          ...
          // console.log("res.payload.actionPlanRequired",res.payload.actionPlanRequired);
          if (res.payload) {
