...
@IonicPage()
@Component({
  selector: "page-auditfirewall",
  templateUrl: "auditfirewall.html"
})
export class AuditfirewallPage {
  actionPlan: any;
  text: any;
  ...
  // Reseting the supplier list after the back navigations
  ionViewWillEnter() {
    this.supplier_id = "";
  }
  ...
  supplierChange(event) {
    if (event != "") {
      this.supplier_id = event;
      console.log("supplier", event);
      this.getAuditList(this.supplier_id);
    }
  }
  ...
  monthChange(event) {
    if (event != "") {
      ...
      // let monthData = this.monthlist.map(monthObject => {
      //   if(monthObject.month  == event){
      //     return monthObject
      //   }
      // });
      // this.nodatafound = '';
  ...
  public getSubActivity() {
    this.utility.showLoader("Fetching data...");
    var data = { ActivityName: "Firewall" };
